...
# simulation to detect and handle collisions between objects
TOP_SIDE    = 0
BOTTOM_SIDE = 2
LEFT_SIDE   = 3
RIGHT_SIDE  = 1
...
# adjust the position of the sprite based on which side it collided with another sprite.
...
#stops the music and kills the Player sprite.
    def kill(self):
...
# updating the player's animation, movement, and various timers and attributes. 
    def update(self):
        self.frame += 1
        self.still_timer -= 1
        self.hit_timer -= 1
...
#represents a basic platform and has the images.
...
#used to determine which sides of the platform are collidable.
...
#these classes define the behavior of cloud objects in a game
...
#defines the behavior of a fire-breathing Bower character.
...
#defines the behavior of a platform that moves up and down.              
...
# represents a moving underground platform that can collide with the player. 
...
# represent a hill object that can be collided with by the player
...
class Menu(object):
    ...
    def main_loop(self):
        while 1:
            ...
            ren = self.font2.render("Game by Dhurub", 1, (255, 255, 255))
            self.screen.blit(ren, (320-ren.get_width()/2, 80))
